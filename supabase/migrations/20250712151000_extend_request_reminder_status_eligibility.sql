-- =============================================================================
-- EduFlow Phase 2C.2 — Extend Request Reminder Status Eligibility
-- =============================================================================
-- Allow teachers to send reminders while a request is new OR in_progress.
-- Completed and rejected requests remain ineligible. Cooldown is unchanged and
-- continues to apply per request regardless of later status changes.
...
BEGIN
  ...
  -- Status condition change (Phase 2C.2):
  --   BEFORE: status = 'new'
  --   AFTER:  status IN ('new', 'in_progress')
...
COMMENT ON FUNCTION public.send_request_reminder(UUID) IS
    'Teacher action: send a reminder for a new or in-progress request and notify institution administrators.';
