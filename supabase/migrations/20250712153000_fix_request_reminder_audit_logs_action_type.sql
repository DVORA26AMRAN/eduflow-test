-- =============================================================================
-- EduFlow Phase 2C.4 — Fix Request Reminder audit_logs.action_type
-- =============================================================================
-- Discovered public.audit_logs columns on development:
--   id, institution_id, actor_user_id, action_type, entity_type, entity_id,
--   metadata, created_at
-- action_type is NOT NULL. The legacy action column was renamed to action_type.
--
-- Established action values (from prior writers):
--   institution_logo_uploaded | institution_logo_removed | institution_logo_replaced
--   request_reminder_sent (original send_request_reminder action value)
--
...
COMMENT ON FUNCTION public.send_request_reminder(UUID) IS
    'Teacher action: send a reminder for a new or in-progress request, notify administrators, and write audit_logs.action_type.';
