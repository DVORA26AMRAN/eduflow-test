/**
 * Edge Function mirror of src/utils/googleOAuthCrypto.ts — keep algorithms in sync.
 */
...
/**
 * Dual-key decrypt: prefer row encryption_key_id, fall back across active + previous.
 * Production rotation uses previous as decrypt_only until re-encrypt completes.
 */
...
export function redactSecretsForLog(value: unknown): unknown {
  if (typeof value === 'string') {
    return value
      ...
      .replace(/hangoutLink":\s*"[^"]+"/gi, 'hangoutLink":"[REDACTED_MEET_URL]"')
  }
